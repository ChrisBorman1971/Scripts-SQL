-- FindAutogrowthEvents.sql    rm     08/03/16
--
-- Look for Autogrowths of data and log files in the last day/ week
...
SELECT 	ServerName, DatabaseName, FileName, SessionLoginName, HostName,
	CASE EventClass WHEN 92 Then ' Data File Autogrowth'
                	WHEN 93 Then ' Log File Autogrowth'
	END as GrowthType,
	StartTime, EndTime 
  FROM ::fn_trace_gettable(@filename, DEFAULT) AS ftg 
    WHERE (EventClass = 92  -- Date File Auto-grow
           OR EventClass = 93) -- Log File Auto-grow
--      AND StartTime > DATEADD(dy,-1,GETDATE());  -- last day
      AND StartTime > DATEADD(dy,-7,GETDATE());  -- Last week



/*  The above is derived from the below, which sets up emails to alert

-- http://www.sqlservercentral.com/scripts/autogrowth/110001/
...
-- If there are autogrows in the last day
*/
