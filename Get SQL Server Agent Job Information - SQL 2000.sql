...
--		[sJOB].[description] AS [JobDescription], 
		...
		[sSVR].[srvname] AS [OriginatingServerName], 
		[sJSTP].[step_id] AS [JobStartStepNo], 
		[sJSTP].[step_name] AS [JobStartStepName], 
		CASE
			WHEN [sSCH].[schedule_id] IS NULL THEN 'No'
			ELSE 'Yes'
		END AS [IsScheduled], 
		[sSCH].[schedule_id] AS [JobScheduleID], 
...
FROM	[msdb].[dbo].[sysjobs] AS [sJOB] LEFT JOIN [master].[dbo].[sysservers] AS [sSVR] ON [sJOB].[originating_server] = [sSVR].[srvname]
		...
		LEFT JOIN [master].[dbo].[sysusers] AS [sDBP] ON [sJOB].[owner_sid] = [sDBP].[sid]
		LEFT JOIN [msdb].[dbo].[sysjobschedules] AS [sJOBSCH] ON [sJOB].[job_id] = [sJOBSCH].[job_id]
		LEFT JOIN [msdb].[dbo].[sysjobschedules] AS [sSCH] ON [sJOBSCH].[schedule_id] = [sSCH].[schedule_id]
ORDER BY	[JobName]
